...
//            OpenSource.Utilities.InstanceTracker.Add(this);
        ...
        public void PipelineRequest(Uri Resource, object Tag)
        {
            ...
            {
                ContinueRequest(new IPEndPoint(IPAddress.Parse(Resource.Host), Resource.Port), HTTPMessage.UnEscapeString(Resource.PathAndQuery), Tag, null);
        ...
        private void GetHostByNameSink(IAsyncResult result)
        {
            ...
            ContinueRequest(new IPEndPoint(e.AddressList[0], Resource.Port), HTTPMessage.UnEscapeString(Resource.PathAndQuery), Tag, null);
        }
        ...
        private void ContinueRequest(IPEndPoint dest, string PQ, object Tag, HTTPMessage MSG)
        {
            ...
            lock (TagQueue)
            {
                ...
                if (s == null)
                {
                    ...
                    {
                        s = new HTTPSession(new IPEndPoint(localif, 0), ProxySetting, new HTTPSession.SessionHandler(CreateSink), new HTTPSession.SessionHandler(CreateFailedSink), null);
                    }
                    else
                    {
                        s = new HTTPSession(new IPEndPoint(localif, 0), dest, new HTTPSession.SessionHandler(CreateSink), new HTTPSession.SessionHandler(CreateFailedSink), null);
                    }
                }
                else
                {
