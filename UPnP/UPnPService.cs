...
namespace OpenSource.UPnP
{
    ...
    public class UPnPStateVariable : ICloneable
    {
        ...
        {
            if (SendEvent && Value != null && OnModified != null)
    ...
    public sealed class UPnPService : ICloneable
    {
        ...
        internal UPnPService(double version)
        {
            ...
            else
            {
                ...
                Minor = 0;
                if (TempNum.DCOUNT() == 2)
                    int.TryParse(TempNum[2], out Minor);
            }
        }
        ...
        {
            if (RemoteMethods.ContainsKey(ActionName) == false)
                return null;
        ...
        public void InvokeAsync(String MethodName, UPnPArgument[] InVarArr, object Tag, UPnPService.UPnPServiceInvokeHandler InvokeCallback, UPnPService.UPnPServiceInvokeErrorHandler ErrorCallback)
        {
            HTTPMessage request = new HTTPMessage();
            if (InVarArr == null)
            {
            ...
            //request.AddTag("X-AV-Client-Info", "av =\"5.0\"; cn=\"Sony Corporation\"; mn=\"PC TV Plus\"; mv=\"6.4\";");
            //request.AddTag("X-PARENTALLOCK", "Tuner/1.0");
            //request.AddTag("X_JLABS_PLR", "");
            request.AddTag("User-Agent", "10.0.19045 2/, DLNADOC/1.50 UPnP/1.0, Portable SDK for UPnP devices/1.6.13");
        ...
        private void HandleInvokeRequest(HTTPRequest sender, HTTPMessage response, object Tag)
        {
            ...
            if (((UPnPAction)RemoteMethods[state.MethodName]).HasReturnValue == true)
            {
                ...
                RetVal = CreateObjectInstance(RetType, (string)((UPnPArgument)VarList[0]).DataValue);
                StartIDX = 1;
            }
        ...
        public void RemoveMethod(UPnPAction action)
        {
            ...
            foreach (UPnPArgument arg in A.ArgumentList)
            {
                ...
                if (arg.RelatedStateVar.GetAssociations().Length == 0)
                    RemoveStateVariable(arg.RelatedStateVar);
            }
                        ...
                        for (int x = 0; x < EventURLS.Length; ++x)
                        {
                            try
                            {
                                ...
                                if (dest.AddressFamily == AddressFamily.InterNetwork)
                                    Packet.AddTag("Host", EventURLS[x].Host + ":" + EventURLS[x].Port.ToString());
                                if (dest.AddressFamily == AddressFamily.InterNetworkV6)
                                    Packet.AddTag("Host", "[" + RemoveIPv6Scope(EventURLS[x].Host) + "]:" + EventURLS[x].Port.ToString());
                                Packet.AddTag("SID", sinfo.SID);
